class ScatterPlotCypressSun {
    ...
    render(selection) {
        ...
        circles
            ...
            .attr("fill", d => { 
                if(selection){
                    if(bar_chart_cypress.xscale(d.date)>=selection[0] && bar_chart_cypress.xscale(d.date)<=selection[1]){ 
                        return "red"; 
                    }
                }
                return "black" }
        ...
        circles
            .on('mouseover', (e,d) => {
                d3.select('#tooltip')
                    .style('opacity', 1)
                    .html(`<div class="tooltip-label">${d.date}</div>(${d.cypress}, ${d.sun})`);
            })
            .on('mousemove', (e) => {
                const padding = 10;
                d3.select('#tooltip')
